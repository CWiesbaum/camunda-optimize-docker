...
if [[ $1 != "standalone" ]]; then
	...
	echo
	exec /bin/bash "$BASEDIR/server/elasticsearch-6.0.0/bin/elasticsearch" &

    URL="http://${ELASTIC_HOST:-localhost}:9200/_cluster/health?wait_for_status=yellow&timeout=10s"
...
OPTIMIZE_CLASSPATH=$BASEDIR/environment:$BASEDIR/plugin/*:$BASEDIR/optimize-backend-2.0.0.jar
...
echo
echo -e "\t http://${OPTIMIZE_HOST:-localhost}:8090/login"
echo
